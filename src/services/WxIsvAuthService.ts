...
const wxImage = new class {
  encode (uri: string = '') {
    return _.toString(uri).replace(/https?:\/\//, 'bs_wx/')
      .replace(/\/\d{1,3}$/, '')
      .replace(/\/$/, '')
  }

  decode (uri: string = '') {
    uri = _.toString(uri).replace(/^bs_wx\//, '')
      .replace(/\/\d{1,3}$/, '')
      .replace(/\/$/, '')
    return 'https://' + uri + '/0'
  }
...
export class WxIsvAuthService extends WxIsvTokenService {
  ...
  async getAppInfo (appWxaId: string) {
    ...
    return appInfo as WxIsv.WxIsvAuthAppInfo
  }
  ...
  async refreshAppTokenInfo (appWxaId: string, refresh_token: string) {
    ...
    return await this.request('POST', '/cgi-bin/component/api_authorizer_token', body, { component_access_token: await this.getToken() }) as WxIsv.WxIsvAuthRefreshInfo
  }
  ...
  async jscode2session (appId: string, js_code: string) {
    ...
    return await this.request('GET', '/sns/component/jscode2session', {}, { appid: appId, js_code, grant_type: 'grant_type', component_access_token, component_appid }) as { openid: string, sessionKey: string }
  }
