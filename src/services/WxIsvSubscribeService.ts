...
export class WxIsvSubscribeService extends WxIsvServiceBase {
  ...
  async getPubTemplateKeywords (accessToken: string, tid: string) {
    return await this.request('GET', '/wxaapi/newtmpl/getpubtemplatekeywords', {}, { access_token: accessToken, tid }) as WxIsv.WxIsvTmplKeywordResponse
  }
  ...
  async addTemplate (accessToken: string, tid: string | number, kidList: number[], sceneDesc: string) {
    return await this.request('POST', '/wxaapi/newtmpl/addtemplate', { tid, kidList, sceneDesc }, { access_token: accessToken }) as WxIsv.WxIsvAddTemplateResponse
  }
  ...
  async delTemplate (accessToken: string, priTmplId: string) {
    return await this.request('POST', '/wxaapi/newtmpl/deltemplate', { priTmplId }, { access_token: accessToken }) as WxIsv.WxIsvNormalResponse
  }
  ...
  async send (accessToken: string, { openId, templateId, data, miniProgramState, page }: { openId: string, templateId: string, data: {}, miniProgramState: 'developer' | 'trial' | 'formal', page: string }) {
