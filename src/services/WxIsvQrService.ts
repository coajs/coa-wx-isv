...
export class WxIsvQrService extends WxIsvServiceBase {
  ...
  async getCommitQrImage (accessToken: string, path: string) {
    ...
    return await this.request('GET', '/wxa/get_qrcode', {}, { access_token: accessToken, path }, { responseType: 'stream' })
  }

  // 获取小程序码（无数量限制）
  async getWxaImageUnlimited (accessToken: string, data: { scene: string, page: string, is_hyaline: boolean }) {
    return await this.request('POST', '/wxa/getwxacodeunlimit', { width: 1280, auto_color: false, ...data }, { access_token: accessToken }, { responseType: 'stream' })
  }

  // 获取小程序码（有数量限制）
  async getWxaCodeLimited (accessToken: string, data: { path: string, is_hyaline: boolean }) {
    return await this.request('POST', '/wxa/getwxacode', { width: 1280, auto_color: false, ...data }, { access_token: accessToken }, { responseType: 'stream' })
  }
