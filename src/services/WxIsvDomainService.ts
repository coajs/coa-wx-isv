...
export class WxIsvDomainService extends WxIsvServiceBase {
  ...
  async get(accessToken: string) {
    return (await this.request('POST', '/wxa/modify_domain', { action: 'get' }, { access_token: accessToken })) as WxIsv.WxIsvDomainModify
  }

  // 设置小程序服务器域名
  async update(
    accessToken: string,
    domian: { requestDomain: string[]; wsRequestDomain: string[]; uploadDomain: string[]; downloadDomain: string[] },
    action: 'set' | 'add',
